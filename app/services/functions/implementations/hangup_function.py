...
async def actions_call(call_sid: str):
    ...
    try:
        ...
        return "Se colgó la llamada de manera exitosa."
            ...
            logger.warning(f"La solicitud tardó demasiado en responder, colgar: https://websockets.ccc.uno/api/v1/autoagent - {gettime()} - call_sid {call_sid}")
        ...
        logger.warning(f"Error en colgar: {str(e)}| Traceback: {formatted_traceback} - {gettime()} - call_sid {call_sid}")
        return {"error": str(e)}