...
async def get_customer_debt(call_sid):
    """Obtener la deuda del CUSTOMER.
    ...
    debt_amount_column = 3  # Assuming debt amount is in the fourth column (0-based index)
    for row in values:
        if len(row) > debt_amount_column and row[1] == phone_number:
            debt_amount = row[debt_amount_column]
            return debt_amount
