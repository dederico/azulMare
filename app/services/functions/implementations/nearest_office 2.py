...
async def find_nearest_government_office(latitude: float, longitude: float, office_type: str) -> Dict[str, Any]:
    """
    Encuentra la oficina gubernamental más cercana a la ubicación del usuario.
    
    latitude (number): Latitud de la ubicación del usuario
    longitude (number): Longitud de la ubicación del usuario
    office_type (string): Tipo de oficina gubernamental ('registro_civil' o 'centro_comunitario')
    
    Returns:
        dict: Resultado con la oficina más cercana y las alternativas ordenadas por distancia
    """
    try:
        # Asegurarse de que los parámetros son del tipo correcto
        lat = float(latitude)
        lon = float(longitude)
        office = str(office_type).lower()
        
        # Pasar los argumentos necesarios a la función
        result = get_nearest_office(user_lat=lat, user_lon=lon, office_type=office)
        
        # Convertir el resultado a un formato seguro para JSON
        safe_result = {
            "success": True,
...
def get_nearest_office(user_lat: float, user_lon: float, office_type: str) -> Dict[str, Any]:
    """
    ...
    """
    # Definir las ubicaciones de las oficinas
    if office_type == 'registro_civil':
        offices = [
            {
    ...
    elif office_type == 'centro_comunitario':
        offices = [
            {
